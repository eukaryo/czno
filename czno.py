...
def getMinBarrierPathDijkstra(sequence, structure1, structure2):
    ...
    #exactは[x]のResultが確定したかどうか
    exact = np.full(2**H, False)
    ...
    while len(Dijkstra) > 0:
        (solution, _, index) = heapq.heappop(Dijkstra)
        if exact[index] == True: continue
        exact[index] = True
        result[index] = solution
        if index == np.uint64(2 ** H - 1): break
        for b in [np.uint64(2**i) for i in range(H) if np.bitwise_and(index, np.uint64(2**i)) == np.uint64(0) and np.bitwise_and(index, prerequisite_flag[i]) == prerequisite_flag[i]]:
            new_index = np.bitwise_or(index, b)
	    if exact[new_index] == True: continue
            if energy[new_index] == np.inf:
                energy[new_index] = energyOfStr(sequence, makeDotBracket(structure1, bp_set1, [diff_pos_list[x] for x in range(H) if np.bitwise_and(new_index, np.uint64(2**x)) != np.uint64(0)]))
            new_result = max(result[index], energy[new_index])
	    if new_result >= result[new_index]: continue
	    result[new_index] = new_result
            count -= 1
    ...
    while index > np.uint64(0):
        ...
        for i in range(H):
            ...
            if exact[back_index] == False: continue
